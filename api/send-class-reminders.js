...
// Cron: send reminder emails the day before class.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const schedule of schedules) {
      ...
      const location = schedule.classes?.location || 'Location TBD';
      ...
      const { data: bookings, error: bErr } = await supabase
        .from('bookings')
        .select('id, contact_email, contact_first_name, number_of_participants, reminder_email_id, email_unsubscribed')
        .eq('class_schedule_id', schedule.id)
        .eq('status', 'Confirmed')
        .or('reschedule_status.is.null,reschedule_status.neq.Pending Reschedule');
      if (bErr) {
        console.error(`[REMINDER-CRON] Fetch bookings failed for schedule ${schedule.id}:`, bErr);
      } else {
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #F0F4F8; border: 2px solid #2C3E50; border-radius: 8px; padding: 25px; margin: 30px 0;">
    ...
    <table style="width: 100%; font-size: 16px;" cellpadding="8" cellspacing="0">
      ...
      <tr><td style="padding: 12px 8px; border-bottom: 1px solid #CBD5E0; font-weight: bold; color: #2C3E50;">Location:</td><td style="padding: 12px 8px; border-bottom: 1px solid #CBD5E0; color: #1E293B;">${location}</td></tr>
  ...
  <div style="background: #F8F9FA; border: 1px solid #D1D5DB; border-radius: 8px; padding: 25px; margin: 30px 0; text-align: center;">
    ...
    <p style="color: #6B7280; font-size: 14px; line-height: 1.6; margin-top: 12px;">
      ...
      Walnut Creek, CA | (925) 532-9953
            ...
            console.error(`[REMINDER-CRON] Student email failed for booking ${booking.id}:`, err);
      ...
      const { data: assignments, error: aErr } = await supabase
        ...
        .eq('class_schedule_id', schedule.id);
      if (aErr) {
        console.error(`[REMINDER-CRON] Fetch TA assignments failed for schedule ${schedule.id}:`, aErr);
        continue;
      }
...
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #F5F3FF; border: 2px solid #553c9a; border-radius: 8px; padding: 25px; margin: 30px 0;">
    ...
    <table style="width: 100%; font-size: 16px;" cellpadding="8" cellspacing="0">
      ...
      <tr><td style="padding: 12px 8px; font-weight: bold; color: #553c9a;">Location:</td><td style="padding: 12px 8px; color: #1E293B;">${location}</td></tr>
    </table>
  </div>
  ...
  <div style="background: #F8F9FA; border: 1px solid #D1D5DB; border-radius: 8px; padding: 25px; margin: 30px 0; text-align: center;">
    ...
    <p style="color: #6B7280; font-size: 15px; margin: 8px 0;">Streetwise Self Defense</p>
          ...
          console.error(`[REMINDER-CRON] TA email failed for ${person.email}:`, err);
    ...
    return res.json({ success: true, classesFound: schedules.length, emailsSent: totalEmailsSent, taEmailsSent: totalTaEmailsSent, results });
  } catch (error) {
