...
`;
            ...
            // Send SMS reminder if phone number exists and Twilio is configured
            const contactPhone = booking.fields['Contact Phone'];
            const TWILIO_ACCOUNT_SID = process.env.TWILIO_ACCOUNT_SID;
            const TWILIO_AUTH_TOKEN = process.env.TWILIO_AUTH_TOKEN;
            const TWILIO_PHONE_NUMBER = process.env.TWILIO_PHONE_NUMBER;
            
            if (contactPhone && TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_PHONE_NUMBER) {
              try {
                const { default: twilio } = await import('twilio');
                const twilioClient = twilio(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN);
                
                // Format phone number (remove any formatting, ensure it starts with +1)
                let formattedPhone = contactPhone.replace(/[\s\-\(\)]/g, '');
                if (!formattedPhone.startsWith('+')) {
                  formattedPhone = '+1' + formattedPhone;
                }
                
                // Create short, clear SMS message
                const smsMessage = `Streetwise Self Defense: Hi ${contactFirstName}! Your class is tomorrow at ${displayStartTime}. Complete your waiver: ${waiverUrl} Questions? Call/text (925) 532-9953`;
                ...
                console.log(`[REMINDER-CRON] Sent SMS to ${formattedPhone} for booking ${booking.id}`);
              } catch (smsErr) {
                console.error(`[REMINDER-CRON] Failed to send SMS for booking ${booking.id}:`, smsErr.message);
                // Don't fail the whole process if SMS fails - email was sent successfully
              }
            }
